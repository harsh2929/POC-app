...
import 'package:AIApp/state/warning.dart';
import 'package:AIApp/theme.dart';
import 'package:AIApp/widget/circle_button.dart';
import 'package:AIApp/widget/circle_icon_button.dart';
...
class RecipeCard extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final selectedRecipeNotifier =
        ref.read(SelectedRecipeNotifier.provider.notifier);
    final historyNotifier = ref.read(HistoryNotifier.provider.notifier);
    final isFavorite = ref.watch(HistoryNotifier.provider.select((e) =>
        e.firstWhereOrNull((f) => f.recipe.title == recipe.title)?.favorite ??
        false));
    final warningNotifier = ref.read(WarningNotifier.provider.notifier);
    return Container(
      margin: const EdgeInsets.only(
          left: M.normal, right: M.normal, bottom: M.small),
                  ...
                  SizedBox(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        RoundButton(
                            onPressed: () => selectedRecipeNotifier.store(
                                recipe, fromHistory),
                            title: "Cook now",
                            rightIcon: Container(
                              width: 24,
                              ...
                              decoration: const BoxDecoration(
                                  shape: BoxShape.circle, color: C.white),
                              child: const VecPic(
                                "up_arrow",
                                color: C.front,
                                iconSize: 8,
                        ...
                        CircleButton(
                            ...
                            onPressed: () =>
                                historyNotifier.toggleFavorite(recipe)),
                      ],
                    ),
                  ),
                ],

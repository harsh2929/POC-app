...
import 'package:AIApp/screens/old_refine/one_subtree.dart';
...
class OneCategory extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final newPath =
        ref.watch(PathNotifier.provider.select((s) => s[category] ?? []));
    ...
    final pathToTrees =
        trees.length < previousTrees.length ? previousTrees : trees;
    final path =
        trees.length < previousTrees.length ? previousPath.split(",") : newPath;

    final controller = usePageController(initialPage: trees.length - 1);

    useEffect(() {
      if (controller.hasClients) {
        controller.animateToPage(newPath.length,
    ...
    return ShaderMask(
      ...
      child: PageView(
        ...
        children: pathToTrees.mapIndexed((index, t) {
          final pathToRender = path.isNotEmpty
              ? [...path.sublist(pathToTrees.length - 1 - index)]
              : <String>[];

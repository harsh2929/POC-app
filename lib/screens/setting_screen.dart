...
import 'package:AIApp/state/path.dart';
...
import 'package:AIApp/state/tracking.dart';
import 'package:AIApp/state/tree.dart';
import 'package:AIApp/state/user_email.dart';
import 'package:AIApp/state/user_name.dart';
import 'package:AIApp/theme.dart';
import 'package:AIApp/utils/compose.dart';
import 'package:AIApp/widget/confirm_dialog.dart';
import 'package:AIApp/widget/input_text.dart';
import 'package:AIApp/widget/option_row.dart';
import 'package:AIApp/widget/option_toggle_row.dart';
          ...
          child: Column(
            ...
            children: [
              ...
              InputText(
                  ...
                  onFieldSubmitted: (f) =>
                      userNameNotifier.update(controller.text)),
              const SizedBox(height: M.normal),
              OptionToggleRow(
                  selected: tracking,
                  title: "Analytics",
                  onToggle: trackingNotifier.toggle),
              OptionRow(
                  selected: false,
                  title: "Clear history",
                  onSelect: () =>
                      confirmDialog(context, onSure: historyNotifier.cleanup)),
              OptionRow(
                  selected: false,
                  title: "Clear session",
                  onSelect: () =>
                      confirmDialog(context, onSure: sessionIdNotifier.reset)),
              if (isLoggedIn)
                OptionRow(
                  selected: false,
                  title: "Delete my profile",
                  onSelect: () => confirmDialog(context,
                      onSure: compose(cleanup, Amplify.Auth.deleteUser)),
                ),
              if (isLoggedIn)
                OptionRow(
                  selected: false,
                  title: "Log out",
                  onSelect: () => confirmDialog(context,
                      onSure: compose(cleanup, Amplify.Auth.signOut)),
                ),
              ...
              if (!isLoggedIn)
                OptionRow(
                  ...
                  title: "Log in",
                  onSelect:
                      compose(startupNotifier.login, skipAuthNotifier.unskip),
                ),
              const Spacer(),
            ],
          ),

...
import 'package:AIApp/ext/string_ext.dart';
import 'package:AIApp/model/prompt_category_request.dart';
import 'package:AIApp/model/prompt_decompose_request.dart';
import 'package:AIApp/model/prompt_response.dart';
import 'package:AIApp/model/tree.dart';
import 'package:AIApp/state/access_token.dart';
import 'package:AIApp/state/error.dart';
import 'package:AIApp/state/top_category.dart';
...
class CategoryFetchedNotifier extends StateNotifier<bool> {
  final Ref ref;
  static final provider = StateNotifierProvider<CategoryFetchedNotifier, bool>(
      CategoryFetchedNotifier.new);
...
class TreeNotifier extends StateNotifier<List<Tree>> {
  ...
  static final provider =
      StateNotifierProvider<TreeNotifier, List<Tree>>(TreeNotifier.new);
  ...
  refresh({String promptMaybe = "", bool userDefaults = true}) async {
    ...
    final categoryFetchedNotifier =
        ref.read(CategoryFetchedNotifier.provider.notifier);

    final currentPrompt = ref.read(PromptNotifier.provider);
    final prompt =
        promptMaybe.isBlank ? currentPrompt.current : promptMaybe.trim();
    ...
    try {
      ...
      final originalMap = {
        for (var e in state) e.category: e.preference.first.replaceAll("_", " ")
      };
  ...
  getCategories() async {
    final promptStruct =
        state.map((e) => "${e.category}=${e.preference.first}").join(";");
    ...
    try {
      ...
      final newTrees = response.results
          .where((e) => existingTopCategories.contains(e.category));

      // merge new results with tree root, keeping the original option at the top
      state = state.mapp((e) {
        final newTree =
            newTrees.firstWhereOrNull((f) => f.category == e.category);
  ...
  togglePreference(Tree selectedTree, String preference) {
    ...
    if (alreadyContains &&
        selectedTree.preference.length == 1 &&
        path.isEmpty) {
      return;
    }
    final updatedPreference = alreadyContains

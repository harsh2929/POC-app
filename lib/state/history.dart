...
class HistoryNotifier extends HydratedStateNotifier<List<HistoryEntry>> {
  ...
  @override
  List<HistoryEntry> fromJson(Map<String, dynamic> json) => json[key] == null
      ? []
      : (json[key] as List<dynamic>).mapp((m) => HistoryEntry.fromJson(m));
  ...
  addAll(List<Recipe> recipes, String prompt) {
    ...
    final newEntries = recipes
        ...
        .mapp((e) => HistoryEntry(
            recipe: e,
            created: DateTimeExt.timestamp(),
            favorite: false,
            prompt: prompt));
    state = [...newEntries, ...state];
  }

  toggleFavorite(Recipe recipe) => state = state.mapp((e) =>
      e.recipe.title == recipe.title ? e.copyWith(favorite: !e.favorite) : e);

  remove(Recipe recipe) =>
      state = [...state.where((e) => e.recipe.title != recipe.title)];

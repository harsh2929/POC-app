...
import 'package:AIApp/env/.env.dart';
import 'package:AIApp/model/prompt_input.dart';
import 'package:AIApp/model/replicate_request.dart';
import 'package:AIApp/model/replicate_response.dart';
import 'package:AIApp/state/error.dart';
import 'package:AIApp/state/thumbnail.dart';
...
final thumbnailAPI =
    FutureProvider.autoDispose.family<String, String>((ref, prompt) async {
  ...
  try {
    var result = await dioClient.safePost(
        'https://api.replicate.com/v1/predictions', ReplicateResponse.fromJson,
        data: payload.toJson(),
        options: Options(
          headers: {
    ...
    while (result.status != "succeeded" || (result.output?.isEmpty ?? true)) {
      ...
      result =
          await dioClient.safeGet(result.urls.get, ReplicateResponse.fromJson,
              options: Options(
                headers: {
                  "Authorization": "Token ${Environment.replicateApiKey}",
                  "Content-Type": "application/json",
